...
function showcase_opening_page_header() {
	if ( ( is_front_page() && is_active_sidebar( 'front-page-hero' ) ) || ( is_page() && !is_page_template( 'page_landing.php' ) ) || is_single() || is_archive() || is_home() ) {
		...
		if ( is_front_page() && is_active_sidebar( 'front-page-hero' ) ) {
			$hero_image = get_option( 'showcase-hero-image', sprintf( '%s/images/unique-banner-image-no-logo.jpg', get_stylesheet_directory_uri() ) );
					...
					if ( is_front_page() && is_active_sidebar( 'front-page-hero' ) ) {
						...
						if ( is_active_sidebar( 'front-page-intro' ) ) {
							genesis_widget_area( 'front-page-intro', array(
								'before' => '<div id="front-page-intro" class="front-page-intro flexible-widget-area"><div class="wrap"><div class="flexible-widgets widget-area' . showcase_widget_area_class( 'front-page-intro' ) . '">',
								'after'  => '</div></div></div>',
							) );
						}
						...
						if ( is_category() || is_tag() || is_tax() ) {
							...
							echo single_term_title( '<h3>', '</h3>', true );
							/**
							  * This is a weird hack to fix the non-closing h3 above, for some reason
							  *	it's including the paragraph below, so I'm echoing a closing h3 tag.
							  */
							echo '</h3>';
						...
						$posts_page_id = get_option('page_for_posts');
						echo '<h3>' . get_the_title($posts_page_id) . '</h3>';
						...
						echo showcase_bar_to_br( the_title( '<h3>', '</h3>', false ) );
					}
